...
function Chat() {
  ...
  const [messages, setMessages] = useState([
    {
      text: "مرحباً، أنا دايسون أي، كيف يمكنني مساعدتك؟",
      timestamp: new Date().toISOString(),
      sender: "gpt",
    },
  ]);
  ...
  const messageAPIdeneme = async (message) => {
    ...
    if (tThread === null) {
      ...
      const abc = "شكراً لك، أنا بخير، كيف حالك؟ كيف يمكنني مساعدتك؟";
      ...
      setMessages((prevState) => [
        ...prevState,
        {
          text: "شكراً لك، أنا بخير، كيف حالك؟ كيف يمكنني مساعدتك؟",
          timestamp: new Date().toISOString(),
          sender: "gpt",
        },
      ]);
  ...
  return (
    <div className="w-screen text-xs flex flex-col h-screen ">
      ...
      >
        <div className="text-lg font-bold ">Dyson Ai</div>
      </div>
      <main
